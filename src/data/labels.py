...
# 원래 라벨은 -1 또는 1이고, 학습 배열에서는 각각 0 또는 1로 표현합니다.
ACTION_TO_INDEX = {-1: 0, 1: 1}

# 위 표를 반대로 뒤집은 표입니다. 예: 0 -> -1, 1 -> 1
...
def rolling_binary_action_labels(
    ...
    """과거 flow 분포와 오늘 flow를 비교해 -1 또는 1 라벨을 만듭니다.
    ...
    # 오늘 flow가 과거 기준선보다 작으면 -1, 기준선 이상이면 1로 표시합니다.
...
def add_action_labels(df: pd.DataFrame, config: dict) -> pd.DataFrame:
    ...
    for investor in config["investors"]:
        ...
        # 모델은 -1/1 같은 액션 값보다 0/1 인덱스를 다루기 쉬워서 함께 저장합니다.
