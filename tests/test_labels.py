...
from src.data.labels import (
    add_action_labels,
    investor_trade_imbalance,
    rolling_binary_action_labels,
    rolling_quartile_action_labels,
)
...
def test_binary_labels_use_past_median_only():
    flow = pd.Series([-0.6, -0.2, 0.2, 0.6, 0.9, -0.9])
    labels = rolling_binary_action_labels(flow, window=4)
...
def test_binary_features_at_t_target_action_at_t_plus_one():
    ...
    config = {
        ...
        "labeling": {"rolling_window": 4, "quantile": 0.5, "label_shift": 1},
    }

    labeled = add_action_labels(frame, config)

    assert labeled.loc[3, "action_foreign"] == 1
    assert labeled.loc[4, "action_foreign"] == -1
    assert labeled.loc[3, "action_idx_foreign"] == 1
